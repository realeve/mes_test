...
test("MES test", async () => {
  ...
  const THREADS = 1; // 每组发送请求数量
  const MAX = 2; // 最多测试请求的实力
  const TIME_TOTAL = 100; // 测试时间窗口长度设定，单位：毫秒
  ...
  // 单个测试
  const invoke = async (method, callback, args = null) => {
